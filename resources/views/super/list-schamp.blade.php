                ...
                <div class="card-body">
                    <h1><span>{{$title}}</span> || <span><a href="{{url('admin/add-schamp')}}">Add Location/Broad Function Wise Safety Champion</a></span> </h1>                    
                     <div class="tab-content">                      
                            <!-- ========= start table ========= -->
                            <div class="row">
                                <div class="searc-hpanel col-sm-6">
                                    <form name="search" id="search" action="javascript:void(0)">
                                    <div class="row">
                                        <div class="col-xl-12">
                                ...
                                <div class="col-sm-6">
                                    <div style="float: right;padding-top: 60px">
                                        <a class="downloadreport-link" href="{{url('admin/location-wise-saftchamp-report?location_code=022')}}">Download Report</a>
                                    </div>
                                </div>
                            </div>
							<br>
...
@section('after_scripts')
    ...
    <script>
        const safetychampTableUrl = '{{url("admin/saftychamp-gettable")}}';
        $.fn.renderDataTable = (locationCode = '022') => {
            ...
            $('#table1').DataTable({
                ...
                serverSide: true,
                ajax:{
                    url: safetychampTableUrl+'/'+locationCode,
                },
                columns: [
        ...
        $(document).ready(function(){
            $.fn.renderDataTable();
            $(document).find('select[name=schamp_loc_code]').val("022");
            $(document).find('select[name=schamp_loc_code]').on('change',function() {
                const slectedLocationCode = $(this).val();
                const urlToDownloadReport = $(".downloadreport-link").attr('href');
                const updatedUrl = new URL(urlToDownloadReport);
                updatedUrl.searchParams.set('location_code', slectedLocationCode);
                $(".downloadreport-link").attr('href',updatedUrl);

            })
            // Location wise search
            $("form[name='search']").submit(function() {
                const selectedLocation = $(this).find('select[name=schamp_loc_code]').val();
                
                $.fn.renderDataTable(selectedLocation);
