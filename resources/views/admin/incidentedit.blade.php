@extends('admin.include.layout') @section('after_styles')
<!-- <link rel="stylesheet" href="{{URL::asset('public/assets/datatable/css/dataTables.bootstrap.min.css')}}"> -->@endsection @section('body') @if ($role == '1' ) @php $read="";@endphp @else @php $read="disabled";@endphp @endif
<!--    reporting -->@if ($role == '2') @php $reado="";@endphp @else @php $reado="disabled";@endphp @endif @if ($role == '3') @php $readonly="";@endphp @else @php $readonly="disabled";@endphp @endif @if ($role == '4') @php $readsh="";@endphp @else @php $readsh="disabled";@endphp @endif @if ($role == '5') @php $readah="";@endphp @else @php $readah="disabled";@endphp @endif @if ($rmedit[0]['rm_comment'] != null && $zaedit[0]['za_comment'] != null ) @php $isComment="";@endphp @else @php $isComment="disabled";@endphp @endif
					...
					<div id="incidentInfo" class="tab-pane fade active show">
						...
						<form method="post" action="{{url('admin/admin-incident_rmcomment')}}" enctype="multipart/form-data">
							...
							<ul style="margin-top: 0;">
								...
								<input type="hidden" name="_token" value="{{csrf_token()}}">
								<li> @if ($role == '1')
									<button type="submit" class="btn-Dark" id="" disabled hidden>Submit </button> @elseif($role == '3')
									<button type="submit" class="btn-Dark" id="" disabled hidden>Submit </button> @elseif($role == '4')
									<button type="submit" class="btn-Dark" id="" disabled hidden>Submit </button> @elseif($role == '5')
									<button type="submit" class="btn-Dark" id="" disabled hidden>Submit </button> @elseif($edit->status_e == '0')
									<button type="submit" name="submit1" value="submit1" class="btn-Dark" id="" disabled hidden>Submit </button>
									@else
										@if($has_comment== NULL)
											<button type="submit" name="submit1" value="submit1" class="btn-Dark" id="">Submit</button>
										@elseif($rm_cmnt_chk!=NULL)
						...
						<form method="post" action="{{url('admin/admin-incident_shcomment')}}" enctype="multipart/form-data">
							...
							<ul style="margin-top: 0;">
								...
								<li class="textarea">
									<div class="form-group">
										<label for="">Comment </label><span style="color:red">*</span> @if(!empty($edit->safety_comment))
								...
								<li class="textarea" id="shneed">
									<div class="form-group">
										<label for="">What Type Of Information You Needed </label><span style="color:red">*</span> @if(!empty($edit->need_informationsh))
								...
								<!--Saheli
									@if($edit->status_e == '0')
										<button type="hidden" class="btn-Dark" name="submitsf" value="submitsf" hidden>Submit </button>
									@else
										<button type="submit" class="btn-Dark" name="submitsf" value="submitsf">Submit </button>
									@endif
								Saheli-->
								@if($edit->status_e == '0')
									<button type="hidden" class="btn-Dark" name="submitsf" value="submitsf" id="" hidden>Submit </button>
								@elseif($role == '2' || $role == '3')
									<button type="hidden" class="btn-Dark" name="submitsf" value="submitsf" id="" hidden>Submit </button>
								@elseif($role == '4' && $edit->status_e == '1')
									<!--saheli-->
									@if($sh_cmnt==NULL)
										@if($ah_cmnt== NULL)
						...
						<form method="post" action="{{url('admin/admin-incident_shcomment')}}" enctype="multipart/form-data">
							...
							<ul style="margin-top: 0;">
								...
								<li>
									<div class="form-group ">
										<label for="">Time of Action </label><span style="color:red">*</span> @if(!empty($edit->safety_time))
						...
						<p><strong >Safety Head comments table</strong></p>
							<table class="table table-striped">
								...
								@if(!empty($shedit))
									...
									@endforeach 
									@foreach($sh_main_cmnt as $sh_main_cmnts)
										<tr>
											<td> {{date('d/m/Y',strtotime($sh_main_cmnts['safety_date']))}} </td>
											<td>{{$sh_main_cmnts['safety_time']}}</td>
											<td>{{$sh_main_cmnts['safety_comment']}}</td>
										</tr> 
									@endforeach 
									</tbody> 
								@endif 
